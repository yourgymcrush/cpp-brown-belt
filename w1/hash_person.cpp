...
struct AddressHasher {
  ...
  size_t operator()(const Address& address) const {
    ...
    return x * x * shash(address.city) + x * shash(address.street) + ihash(address.building);
  }
...
struct PersonHasher {
  ...
  size_t operator()(const Person& person) const {
    ...
    return x * x * x * shash(person.name) + x * x * ihash(person.height) + x * dhash(person.weight) + ahash(person.address);
  }
