...
private:
  ...
  public:
    IndexProxy(Polynomial& polynomial, size_t deegree) : polynomial_(polynomial), deegree_(deegree) {} 

    IndexProxy& operator =(const T& value) {
      if (polynomial_.coeffs_.size() <= deegree_) {
        polynomial_.coeffs_.resize(deegree_ + 1);
      }
      polynomial_.coeffs_[deegree_] = value;
      polynomial_.Shrink();
      return *this;
    }

    bool operator ==(const T value) {
      return std::as_const(polynomial_)[deegree_] == value;
    }

    operator T() const {
      return std::as_const(polynomial_)[deegree_];
    }

    ostream& operator <<(ostream& out) const {
      out << std::as_const(polynomial_)[deegree_];
      return out;
    }
  };
