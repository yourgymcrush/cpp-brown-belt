...
struct BulkTaxApplier {
  double FACTOR = 1.0 - TAX_PERCENTAGE / 100.0;
  uint32_t count = 0;

  BulkTaxApplier() {}
  BulkTaxApplier(size_t proc) {
    FACTOR = 1.0 - proc / 100.0;
    count = 1;
  }
...
struct MoneyState{
  ...
  double GetPureIncome() {
    return income_ - outcome_;
  }
};
...
public:
  ...
  MoneyState Collapse(MoneyState origin, IndexSegment segment) const {
    return {origin.income_ * tax_.ComputeFactor() + add_.delta * segment.length(), origin.outcome_ + spend_.delta * segment.length()};
  }
...
struct SpendRequest : ModifyRequest {
  ...
  void Process(BudgetManager& manager) const override {
    const auto date_segment = MakeDateSegment(date_from, date_to);
    const double daily_income = income * 1.0 / date_segment.length();
    manager.AddBulkOperation(date_segment, BulkMoneySpender{daily_income});
  }
