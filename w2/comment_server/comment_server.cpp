...
public:
  ...
  HttpResponse& AddHeader(string name, string value) {
    mHeaders.push_back({std::move(name), std::move(value)});
    
    return *this;
  }
  HttpResponse& SetContent(string a_content) {
    mContent = std::move(a_content);
...
ostream&  operator << (ostream& output, const HttpCode code) {
  static unordered_map<HttpCode, string> comments = {
    {HttpCode::Ok, "OK"},
    {HttpCode::Found, "Found"},
    {HttpCode::NotFound, "Not found"},
  };
  
  output << "HTTP/1.1 " << (int)code << " " << comments[code] << "\n";
  return output;
}

ostream& operator << (ostream& output, const HttpResponse& resp) {
  output << resp.mCode;
  for (auto &header : resp.mHeaders) {
    output << header.name << ": " << header.value << "\n";
  }
  if (!resp.mContent.empty()) {
    output << "Content-Length: "  << to_string(resp.mContent.size()) << "\n";
  }
  output << "\n";
  if (!resp.mContent.empty()) {
  ...
  HttpResponse ServeRequest(const HttpRequest& req) {
    ...
    if (req.method == "POST") {
      ...
      if (req.path == "/user_comments") {
        ...
        std::string capcha = "What's the answer for The Ultimate Question of Life, the Universe, and Everything?";
        resp.SetCode(HttpCode::Ok).SetContent(capcha);
      } else {
        resp.SetCode(HttpCode::NotFound);
      }
    }
