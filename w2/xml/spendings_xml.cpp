...
vector<Spending> LoadFromXml(istream& input) {
  ...
  const static std::string spendName = "spend";
  const static std::string amountName = "amount";
  const static std::string categoryName = "category";
  ...
  for (const auto& node : root.Children()) {
    ...
    spending.amount = node.AttributeValue<int>(amountName);
    spending.category = node.AttributeValue<std::string>(categoryName);
