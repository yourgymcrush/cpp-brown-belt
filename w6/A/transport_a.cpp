...
struct PostStopRequest : PostRequest, StopRequest
{
    ...
    void ParseFrom(std::string_view input) override
    {
        ...
        stop.coords.second = Reader::convertToDouble(Reader::readToken(input));
    }
...
public:
    ...
    void addStop(const PostStopRequest::Stop& stop)
    {
        ...
        newStop.first = toRad(stop.coords.first);
        newStop.second = toRad(stop.coords.second);
    }
...
private:
    std::unordered_map<Stop, StopCoords> stops;
    std::unordered_map<BusNumber, Route> routes;

    double getDistanceBetween(const StopCoords& lhs, const StopCoords& rhs)
    {
        double res = acos(sin(lhs.first) * sin(rhs.first)
                          + cos(lhs.first) * cos(rhs.first)
                          * cos(abs(lhs.second - rhs.second)))
                     * EarthR * 1000;
        return res;
    }
    void updateRoutes()
    {
        for (auto& bus : routes)
        {
            ...
            for (const auto& stop : route.stops)
            {
                if (!previous.empty()) 
                {
                    route.Length += getDistanceBetween(stops[previous], stops[stop]);
                }
                previous = stop;
            }
        }
